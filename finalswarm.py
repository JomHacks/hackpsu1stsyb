...
def get_input():
    ...
    if pygame.joystick.get_count() > 0:
        ...
        # Left stick
        lr = int(joystick.get_axis(0) * speed)
        fb = int(-joystick.get_axis(1) * speed)

        # Right stick
        ud = int(-joystick.get_axis(3) * speed)
        yv = int(joystick.get_axis(2) * speed)

        # Update drone position based on controller input
        drone_x += lr // 10
        drone_y += fb // 10
...
def draw_map(surface, drone_x, drone_y):
    ...
    coord_rect = coord_text.get_rect(right=WINDOW_WIDTH - 10, bottom=WINDOW_HEIGHT - 10)
...
while running:
    ...
    draw_button("Land (Q/B)" if is_flying else "Take Off (E/A)", 1040, 20, 200, 50, RED if is_flying else GREEN)
    ...
    screen.blit(objects_text, (980, 590))
    for i, obj in enumerate(detected_objects[:2]):  # Display up to 2 objects
        obj_text = small_font.render(obj, True, DARK_BLUE)
        screen.blit(obj_text, (990, 620 + i * 30))

    # Example of printing an alert to the console (you can modify the condition as needed)
    if len(detected_people) > 0:
        print(f"Alert! Person detected at location: X:{drone_x}, Y:{drone_y}. Severity: high.")
